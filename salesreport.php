...
<center>
    ...
    //$produce = $_POST['produce'];
     // formating the dates
      ...
      $date2 = $date->format('Y-m-d');
          ...
          $sql ="SELECT product,code, SUM(quantity) as qsold, unit_price, transaction_date, SUM(quantity*unit_price) as value
            FROM jobuk.sales WHERE transaction_date BETWEEN '$dat1' AND '$dat2' GROUP BY code";
  ...
  $sql ="SELECT product, code,count(code) as typeno  , sum(quantity) as qsold,
            SUM(quantity*unit_price) AS value
            FROM jobuk.sales WHERE transaction_date BETWEEN '$dat1' AND '$dat2'";
            $result = $connect->query($sql);
            if ($result-> num_rows >0) {
              while ($row = $result-> fetch_assoc()) {
               $qinstock= $row["qsold"];
   ...
   echo "<tr>";
  echo "<th colspan='2'>TOTAL ITEMS SOLD IN THE PERIOD </th><td><b>".($qinstock)."</b></td></td>
  <th>TOTAL SALES VALUE </th>
          <td><b> ".($total_sales)."</b></td><td></td><td></td>";
