...
<center>
  ...
  <form action="" method="POST">
        <span class="btn btn-info">
              <label class="hidden-print" style="color: white">Start Date:</label> <input type="Date" required="required" class="btn-info hidden-print" id="date01" name="date1" placeholder="Enter Date to Search" >
            <label class="hidden-print" style="color: white">End Date:</label><input type="Date" required="required" id="date02" name="date2" class="btn-info hidden-print" placeholder="Enter Date to Search">
            <button name="search" value="Generate" id="submit1" class="btn-info hidden-print" onclick="show()"><i class="fa fa-search"></i>
            </button> 
        </span>         
  </form>
</div>
        <div style="max-width: 1000px; margin-top: 4px;">
          <div class="card">
            <div class="card-header">
              ...
              <h2 class="card-title"><a href="addstocksc.php"><button class="btn btn-info">Back</button></a> <b>Product reports for today  <span id="span1"><script type="text/javascript">
                var dat = new Date();
     ...
     // formating the dates
      ...
      $date2 = $date->format('Y-m-d');
          ...
          $sql ="SELECT category, type, quantity, price, transaction_date, (quantity*price) as value
            FROM joe.product
            WHERE username = '$username' AND transaction_date  BETWEEN '$dat1' AND '$dat2'";
