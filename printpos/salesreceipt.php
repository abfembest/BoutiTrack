...
<html lang="en">
 ...
 $sql = "SELECT invoice_id FROM jobuk.temp_sale LIMIT 1";
  $result = $connect->query($sql);
  if ($result-> num_rows >0) {
    while ($row = $result-> fetch_assoc()) {?>

       <?php echo  'Rctno'.": " .$row['invoice_id'].' Cashier: '.$_SESSION["name"]; ?> </div>
  <?php                 
}
} 
?>
<?php
 ...
 $sql = "SELECT invoice_id, product, quantity, unit_price, (quantity *unit_price) AS selling_price, payment_method, transaction_date FROM jobuk.temp_sale";
  $result = $connect->query($sql);
  if ($result-> num_rows >0) {
    while ($row = $result-> fetch_assoc()) {?>
      ...
      <tr>
          ...
          <td class="description"><?php echo $row['selling_price']; ?></td>
             ...
             <?php 
             include 'db_connect.php';
            $sql = "SELECT quantity, unit_price, SUM(quantity*unit_price) AS Amount, SUM(quantity)AS total_quantity FROM jobuk.temp_sale";
            $result = $connect->query($sql);
            if ($result-> num_rows >0) {
              while ($row = $result-> fetch_assoc()) {?>
            ...
            <?php
              ...
              <?php 
              include 'db_connect.php';
 $sql = "SELECT username FROM jobuk.temp_sale LIMIT 1";
  $result = $connect->query($sql);
  if ($result-> num_rows >0) {
    while ($row = $result-> fetch_assoc()) {
